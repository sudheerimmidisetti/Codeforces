...
void solve() {
    ...
    for (int i = 1; i < n; i++) {
        if (a[i] == a[i - 1])
            ...
            mpA[a[i - 1]] = max(mpA[a[i - 1]], cnt);
            cnt = 1;
        }
    }
    mpA[a[n - 1]] = max(mpA[a[n - 1]], cnt);
    ...
    ll maxLen = 0;
    for (auto &x : mpA)
        maxLen = max(maxLen, x.ss + mpB[x.ff]);
    
    for (auto &x : mpB)
        maxLen = max(maxLen, x.ss + mpA[x.ff]);
