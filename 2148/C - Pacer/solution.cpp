...
void solve() {
    ...
    ll i = 0, maxPoints = 0, prevTime = 0, prevSide = 0;
    for (int i = 0; i < n; i++) {
        ll currSide = (prevSide != b[i]);
        if ((a[i] - prevTime) % 2 == currSide)
            maxPoints += (a[i] - prevTime);
        else
            maxPoints += ((a[i] - prevTime) - 1);

        prevTime = a[i];
        prevSide = b[i];
    }

    cout << maxPoints + (m - prevTime) << endl;
}
